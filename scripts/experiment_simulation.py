...
from common import EnvDefault, Cache, CONSTANTS as C, EXPERIMENTAL_CONSTANTS as E
...
def generate_sample_kaon_decay(avg_dlength: float, n: int, rng: Generator) -> NDArray:
    ...
    vertex_positions = np.concatenate((np.zeros((n, 2)), np.reshape(rng.exponential(scale=avg_dlength, size=n), (n, 1))), axis=1)
...
def calculate_num_intersects(z: float, a: NDArray) -> int:
    ...
    0 <= i <= n*(m-1), are intersecting with a disk at position (0, 0, z).
    """
    z_travel = z - a[:, 0, 2]
    ...
    radius_sq_travelled = np.sum((momentum_vecs[:, :2] * travel_time[:, np.newaxis] + np.repeat(a[:, 0, :2], a.shape[1]-1, axis=0))**2, axis=1)
...
def main(args: argparse.Namespace) -> Union[int, tuple[int, Cache]]:
    ...
    print(calculate_num_intersects(2, sample))
    optimal_result = opt.minimize(lambda *a, **k: -calculate_num_intersects(*a, **k), np.array([3,]), args=(sample,))
    print(optimal_result)
    cache.not_angled_ideal_z = optimal_result['x'][0]
    if args.no_write:
        return 0, cache
    else:
...
if __name__ == "__main__":
    ...
    group.add_argument("-c", "--cache-file", action=EnvDefault, type=str, envvar='VALUECACHE', default=r".\data\value_cache.json", help="File path of the value cache.")
    group.add_argument("--cache", help="A base64 representation of a UTF-8 JSON string containing the value cache data. Implies `--no-write`.")
    parser.add_argument("--no-write", action="store_true", help="Do not write out the updated value cache. Additionally, main will return the updated value cache instead.")
